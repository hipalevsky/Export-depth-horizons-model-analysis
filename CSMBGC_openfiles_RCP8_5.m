function [z, z_top, TLAT, TLONG, TAREA, REGION_MASK, mld, POCflux, POCprod, NPP_diat, NPP_diaz, NPP_sp, NPP_sum] = CSMBGC_openfiles_RCP8_5(year); 

%Function takes any year in the RCP 8.5 output (2005 to 2100) and outputs
%data from that year. Currently have commented out additional variables
%that could also be included from output for sake of run time.
...
%     ATM_CO2 = ncread(filename,'ATM_CO2'); %Atmospheric CO2 in ppmv (size is nlon x nlat x time) %NEW - not in Lima 2014 output
%     O2 = ncread(filename,'O2'); %Dissolved oxygen in mmol/m^3 (size is nlon x nlat x z_t x time) %NEW - not in Lima 2014 output
...
%Properties not in RCP 8.5 archive
