...
%m_plot([lonAtl lonAtl],[latminplot_sect latmaxplot_sect],'k--','linewidth',L); hold on;
%m_plot([lonPac lonPac],[latminplot_sect latmaxplot_sect],'k--','linewidth',L); hold on;
m_plot(stn_plot(:,2), stn_plot(:,1),'ko','markerfacecolor','y','markersize',MS*2); hold on;
...
zminplot = 0; zmaxplot = 250;
    zplotid = find(z_new >= zminplot & z_new <= zmaxplot);
...
%     subplot(1,2,i)
%         contourf(glat(latplotid), z_new(zplotid), flipud(rot90(squeeze(dataForSection(latplotid,sections(i),zplotid)))),...
...
for i = 1:length(sections)
    subplot(2,5,[2:5] + 5*(i-1))
        ...
        [Con,h] = contour(glat(latplotid), z_new(zplotid), flipud(rot90(squeeze(dataForSectionb(latplotid,sections(i),zplotid)))),...
...
GapRight = 1;
GapVert = 1.5;
LegendLabels = {'','Minimum annual MLD','Euphotic depth','Particle compensation depth','Maximum annual MLD','100 meters'};
FSLeg = 10;
...
stackedForTSPlotting{1} = POCprod_interp;
stackedForTSPlotting{2} = POCremin_interp;
stackedForTSPlotting{3} = POCflux_interp;
%stackedForTSPlotting{4} = NPP_sum_interp;
stackedForTSPlotting{4} = POCflux_interp./NPP_sum_interp;
...
%% Plot depth horizons over time
tspan = [1:13];
tlabels = {'J','F','M','A','M','J','J','A','S','O','N','D','J'};
title2 = {'Seasonal cycle'};
for i = [9,5,2,6]
    zminplot = 0; zmaxplot = zmaxstn(i)+50; ztickint = 100;
    zplotid = find(z_new >= zminplot & z_new <= zmaxplot);
    zint = z_new(2) - z_new(1);
    ztickmin = find(z_new == ztickint)'
    ztick = [ztickmin:ztickint/zint:max(zplotid)];
    zticklabel = z_new(ztick);
    xmaxplot = ceil(max(POCflux_interp_AnnMean_clim(inds(i,1),inds(i,2),:)));
    ...
    height=6;
    set(gcf,'units','centimeters','position',[x0,y0,width,height])
        subplot(1,3,[1:2]) %plot depth horizons over year for each station
    imagesc(squeeze(POCflux_interp(inds(i,1),inds(i,2),zplotid,tspan))); hold on;
    for j = 1:5
        plot(squeeze(depthCriteria{j}(inds(i,1),inds(i,2),tspan))/zint,'-','color',lineC(j,:),'linewidth',Lwid); hold on;
    end
    ylabel('Depth (m)'); set(gca,'ydir','reverse'); xlim([min(tspan) max(tspan)]);
    xticks(tspan); xticklabels(tlabels); yticks(ztick); yticklabels(zticklabel);
    colormap(C); hc = colorbar; set(hc,'location','westoutside'); ylabel(hc,'POC flux, mol C m^{-2} yr^{-1}')
    title([stnNames{i}]); 
        subplot(1,3,3) %plot mean annual POC flux profile and mean annual depth horizons
    plot(squeeze(POCflux_interp_AnnMean_clim(inds(i,1),inds(i,2),:)),z_new,'b-','linewidth',Lwid*1.5); hold on;
    for j = 1:5
        plot([0:5],repmat(depthCriteria_wAnnMean_clim{j}(inds(i,1),inds(i,2)),6,1),'-','color',lineC(j,:),'linewidth',Lwid); hold on;
    end
    ylabel('Depth (m)'); set(gca,'ydir','reverse'); 
    %xlabel('POC flux, mol C m^{-2} yr^{-1}')
    ylim([0 zmaxplot])
    xlim([0 xmaxplot])
    title('Annual mean POC flux')
end
...
GapRight = 0.4;
GapVert = 0.04;
LegendLabels = {'Seasonal MLD','Euphotic depth','Particle compensation depth','Maximum annual MLD','100 meters'};
FSLeg = 10;
